...
function O(t,e){e?N.profiles={...N.profiles,[e]:t}:N={...N,...t}}function _(t){let e=(t||document.body).querySelectorAll(`[data-${A.join("],[data-")}]${0!=N.fetch?.auto?",a":""}`)||[]
for(let t of e){t.id||(t.id=""+L++)
for(let e in t.dataset){if([`${$}if`,`${$}each`].includes(e)){console.log("Handling conditionals",e),x(t,e,N)
continue}if(!A.includes(e))continue
let n=![`${$}bind`].includes(e),o=`(#${t.id} on ${e})`
t.dataset?.[e]?.split(";").forEach((i=>{let[r,s]=i?.split("->")?.map((t=>t.trim()))||[],l=n?I(r.slice(0,r.indexOf(")"))):[],a=n?r.slice(r.indexOf(")")+1):r,f=a.includes("=>")?a:a.includes("(")&&a.match(/^[^\(]{1,}/)?.[0]||"",u=f?I(a.slice(0,(a.indexOf(")")||-2)+1)):a.split(E)?.map((t=>t.trim()))
...
for(let n of l)e.match(/bind|sync/)?T(t,u,s,n,e,c):b(t,n,N,u[0],e.replace($,""),s)}))}}}function I(t){if(t?.includes("(")){let e=t.match(/[^\(\)]{1,}/g)
t=e?.[e.length-1]||""}return t?.split(E)?.map((t=>t.trim()))||[]}let P={store:(store_name,store_ops)=>(store_ops?.hasOwnProperty("value")||store_ops?.hasOwnProperty("updater")||(store_ops={value:store_ops}),p(store_name,store_ops)),ustore:(store_name,store_ops)=>p(store_name,store_ops),get:store_name=>p(store_name),func:func_name=>MfFn?.get(func_name),funcs:funcs=>{for(let t in funcs)MfFn.set(t,funcs[t])},config:(new_ops,profile_name)=>O(new_ops,profile_name),onTick:t=>{var e;(e=t)&&o.push(e)},register:t=>{"string"==typeof t&&(t=document.querySelector(t)),_(t)}}
globalThis.Mfld=P
let k=globalThis.document?.currentScript?.dataset||{}
if(k?.config)try{O(JSON.parse(k?.config))}catch(t){console.warn("Invalid Mfld params",t)}k?.init&&_(document.querySelector(k.init))
