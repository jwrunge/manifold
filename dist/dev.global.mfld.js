...
for(let e in n.dataset)for(let o of["fetch","trans"])if(e.startsWith(`${t}${o}_`))try{let t=e.split("_")[1],r=n.dataset[e]
r?.match(/\{\[/)&&(r=JSON.parse(r)),parseInt(r)&&(r=parseInt(r)),i[o][t]=r}catch(t){console.error(t)}return i},i=(t,n,o)=>{try{let[i,r]=t?.split(/\s{1,}as\s{1,}/)||[t,"value"],s=`let {$el, $st, $fn, ${n||"$val"}, ${o||"$key"}, $body} = ops;return ${i?.match(/^\s{0,}(function)?\(.{0,}\)(=>)?\s{0,}/)?`(${i})()`:i}`,l=r?.split?.(e)?.map?.((t=>t.trim()))||["value"]||[]
...
return e?n?n.t(t,e):new y(t,e):n||new y(t,e)},m=window
...
v(n),t.observer?.disconnect(),t.toRemove.delete(n),MFLD.mut.delete(e)}})),t.observer.observe(this.p?.parentElement,{childList:!0})),t.toRemove.add(this),MFLD.mut.set(this.p,t)}return e?.upstream?.map((t=>{let e=h(t)
return this.u.add(e),e.$.add(this),e})),this.value=e?.value,this.o=e?.updater,this.m(),this}sub(t,e,o=!0){this.i.set(e||n(),t),o&&t?.(this.value)}update(t){this.h&&clearTimeout(this.h),this.h=setTimeout((()=>{a((()=>{let e="function"==typeof t?t?.(this.value):t,n=g(e)
...
this.update(void 0===t?this.value:t)}}let v=t=>{m.MFLD.st.delete(t?.name||""),t=void 0},w=(e,n,o,s,l,f)=>{console.log(e,"TRIGGER",n,"OPS",o,"HREF",s,"METHOD",l,"FUNC",f)
let u=async n=>{n?.preventDefault(),n?.stopPropagation(),l||(l=(n?.target)?.method||"get")
let u=o?.fetch?.externals?.find((t=>s?.startsWith(t.domain)))||!s.match(/^https?:\/\//)||s.includes(location.origin)?{scripts:!0,styles:!0}:void 0,c=f?.(e,O,N),$=Array.isArray(c)?c[0]:"$form"==c?new FormData(e):c
if(f){let t=Array.isArray(c)?c?.map((t=>h(t).value))||[]:[$]
$=f?.(...t)}let d=await fetch(s,{...o?.fetch?.request||{},headers:{...o?.fetch?.request?.headers,MFLD:"true"},method:l,body:"$form"==c||"string"==typeof $?$:JSON.stringify($)}).catch((t=>{o?.fetch?.err?.(t)})),p=d?.status
if(p&&0==o?.fetch?.onCode?.(p,d))return
let m=await(d?.[o?.fetch?.resType||"text"]())
for(let n of["append","prepend","inner","outer"]){let i=e.dataset[`${t}${n}`]
if(void 0===i)continue
let[r,s]=i?.split("->").map((t=>t.trim()))||[],l=(new DOMParser)?.parseFromString?.(m,"text/html")
...
e.src=t.src,document.head.appendChild(e)})),a({in:l.querySelector(r||"body"),out:s?document.querySelector(s):e,relation:n,ops:o,done:t=>{D(t)}}))}let g=e.dataset?.[`${t}resolve`],y=i(g||"")?.func
y?.(m),r(e,n,s)}
"$mount"==n?u():e.addEventListener(n,u)}
function b(t,e,n,o=!1){let i=o?"previousElementSibling":"nextElementSibling"
return e?.(t)?t:b((n?.(t)||t)?.[i],e,n,o)}let T=(t,e,o)=>h(n(),{upstream:t,updater:()=>e?.({$el:o,$st:F,$fn:A}),scope:o}),M=(t,e,n)=>{T(n,(()=>e?.({$el:t,$st:F,$fn:A})),t)},S=(t,e,n)=>{let o=e=>{n?.({$el:t,$st:F,$fn:A}),r(t,e)}
"$mount"==e?o():t.addEventListener(e,o)},x=(e,n,o,r,s,l)=>{let f,u,c=document.createElement("template"),$=(t=>{let e="TEMPLATE"
...
return"else"==h?.[0]||1==r?.({$el:t,$st:e,$fn:n})}}f=T([...s,...m],p?u:r,$),$.dataset[`${t}cstore`]=f.name,f.sub((t=>{void 0!==t&&a((()=>{b(c?.nextElementSibling,(t=>t?.classList?.contains(`${n}-end`)),(t=>d(t,"out",l,(()=>t?.remove())))),p&&!t||((t,e)=>{if(t instanceof Map)for(const[n,o]of t.entries())e(n,o)
...
if(!p){let s=$?.innerHTML?.replace(/\$:{([^}]*)}/g,((r,s)=>i(s,o[0],o[1])?.func?.({$el:e,$st:F,$fn:A,[o[0]]:t,[o[1]]:n})||""))||""
r?.innerHTML&&(r.innerHTML=s)}for(let e of r.content.children)e?.innerHTML||(e.innerHTML=t),$.before(e),d(e,"in",l)}))}))}))},_={},E=["bind","sync","templ","if","elseif","else","each","get","head","post","put","delete","patch"].map((e=>`${t}${e}`))
...
if(r.id||(r.id=n()),void 0!==r.dataset?.[`${t}promote`]){let[t,e,n,o]="A"==r.tagName?["get",r.href,[],"click"]:[r.method.toLowerCase(),r.action,"$form","submit"]
if(e)continue}for(let n in r.dataset)if(E.includes(n))for(let o of r.dataset?.[n]?.split(";;")||[]){let l=!!n.match(/get|head|post|put|delete|patch/),f=o?.split(/\s*->\s*/g),a=l&&f.pop()||"",u=l||n.match(/sync/)?f.shift()?.match(/[^\(\)]{1,}/g)?.pop()?.split(e)?.map((t=>t.trim())):[]||[],c=f?.[0]||"",$=Array.from(new Set([...c?.matchAll(/\$st\.(\w{1,})/g)].map((t=>t[1])))),{func:d,as:p}=i(c)
if(n.match(/bind|templ/)&&console.log("funcstr",c,"dependencies",$,"setting",o,"href",a,"triggers",u),n.match(/each|templ|if|else/))x(r,n,p||[],d,$,s)
else{u?.length||(u=[""])
for(let e of u)n.match(/bind/)?M(r,d,$):n.match(/sync/)?S(r,e,d):w(r,e,s,a,n.replace(t,""),d)}}}},L={store:
/**!
 ...
 */
t=>{"string"==typeof t&&(t=document.querySelector(t)),D(t)}},O=m.MFLD.$st,N=m.MFLD.$fn
