...
o.textContent=t,e.replaceWith(o),e=o}if(e){let l=Array.isArray(n.trans?.dur)?n.trans?.dur["in"==o?0:1]||n.trans?.dur[0]:n.trans?.dur||0,r=n?.trans?.class||"mf-trans"
e?.classList?.add(r),n.trans?.hooks?.[`${o}-start`]?.(e),"out"==o?i((()=>{t?.size?.(e),l&&(e.style.transitionDuration=`${l}ms`),e.classList?.add(o)})):setTimeout((()=>{i((()=>{l&&(e.style.transitionDuration=`${l}ms`),e?.classList?.add(o),s?.(),i((()=>{e?.classList?.remove(o)}))}))}),n.trans?.swap||0),setTimeout((()=>{i((()=>{"out"==o&&e?.remove(),e?.classList?.remove(r),e?.classList?.remove(o),n.trans?.hooks?.[`${o}-end`]?.(e)}))}),l+("in"==o&&n.trans?.swap||0))}}function l(t){if(!t)return 0
...
for(let[e,o]of f){let n=h(e),s="function"==typeof o?await(o?.(n.value)):o,r=l(s)
if(r!==n.o){n.value=s,n.o=r
for(let e of n.i)t.push(e)
i((()=>{for(let[t,e]of n.t)e?.(n.value,t)}))}}f.clear()
for(let e of t)h(e)&&await h(e).p()
r.forEach((t=>t())),r=[],e(this.value)}),0)}))}async p(){await this.update(await(this.#t?.(this.l?.map((t=>h(t)?.value))||[],this?.value)||this.value))}}function h(t,e){let o=globalThis.Mfld_stores.get(t)
...
let p=/, {0,}/g,b=0
function T(t,e){e?c.profiles={...c.profiles,[e]:t}:c={...c,...t}}function m(t){let e=(t||document.body).querySelectorAll(`[data-${g.join("],[data-")}]${0!=c.fetch?.auto?",a":""}`)||[]
for(let t of e){t.id||(t.id=""+b++)
for(let e in t.dataset){let o=e!=`${d}bind`,n=`(#${t.id} on ${e})`
t.dataset?.[e]?.split(";").forEach((s=>{let l,r=s?.split(/(?:(?:\)|->) ?){1,}/g)||[],a=o?y(r.splice(0,1)[0]):[],f=r[0]?.includes("(")&&r[0]?.match(/^[^\(]{1,}/)?.[0]||"",u=y(r.splice(e==`${d}sync`?1:0,1)[0]),c=y(r[0])
if(o&&!a?.length)throw`No trigger: ${n}.`
if(f){if(l=globalThis[f]||globalThis.Mfld_funcs?.get(f),!l)throw`"${f}" not registered: ${n}`
...
a?.length||(a=[""])
for(let o of a){["bind","sync"].includes(e)&&$(t,o),c?.length||(c=[""])
for(let s=0;s<c.length;s++)if(e==`${d}bind`){let e=()=>{i((()=>{const e=l?.(...g.map((t=>w(h(t.name)?.value,t.path))),t)??w(h(g[0].name||"")?.value,g[0].path)
...
l&&(e=l?.(e,t))
const o=h(g[0]?.name)
void 0!==e&&o?.update?.((t=>g[0]?.path?.length?w(t,g[0]?.path,e):e))}
t.addEventListener(o,e)}}}))}}}function w(t,e,o){let i=t
...
t=e?.[e.length-1]||""}return t?.split(p)?.map((t=>t.trim()))||[]}function $(t,e){let o=async e=>{e?.preventDefault(),e?.stopPropagation()
