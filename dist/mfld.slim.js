...
return this.l.add(e),e.u.add(this),e})),this.value=e?.value,this.t=e?.updater,this.h(),this}sub(t,e,n=!0){this.o.set(e||String(Date.now()+Math.random()),t),n&&t?.(this.value)}async update(t){return new Promise((async e=>{let n="function"==typeof t?(await t)?.(this.value):t,o=c(n)
...
r&&s({in:r.querySelector(o||"body"),out:i?document.querySelector(i):t,relation:e,ops:a,done:t=>{k(t)}})}let $=t.dataset?.[`${h}resolve`],m=v($||"")?.func
m?.(d)}
"$mount"==e?a():t.addEventListener(e,a)}function T(t,e,n,o,i,r){if(i==`${h}bind`){let i=e.map(w),l=()=>{s((()=>{let e=i.map((t=>m(p(t[0])?.value,t[1]))),s=r?.(...e,t)??e[0]
...
for(let t of i)p(t?.[0]||"")?.sub(l)}else if(i==`${h}sync`){let[i,s]=w(n||""),l=()=>{let n=e.map((e=>{let n=(e=e.trim()).split(":")
...
e.innerHTML=u.innerHTML,"TEMPLATE"==u?.tagName&&s({in:e,out:i,relation:"swapinner",ops:o,done:t=>k(t)})})),n=n?.nextElementSibling}}if(e==`${h}each`){i.classList.add("mfld-loop-result")
...
u.innerHTML=a,s({in:u,out:i,relation:"append",ops:o,done:t=>k(t)})}))}))}}let E={},N=/, {0,}/g,A=0,O=["bind","sync","if","each","get","head","post","put","delete","patch"].map((t=>`${h}${t}`))
function _(t,e){e?E.profiles={...E.profiles,[e]:t}:E={...E,...t}}function k(t){let e=(t||document.body).querySelectorAll(`[data-${O.join("],[data-")}]${0!=E.fetch?.auto?",a":""}`)||[]
...
for(let n of l)e.match(/bind|sync/)?T(t,u,s,n,e,c):(s||(s=u[0],u=[]),b(t,n,E,s,e.replace(h,""),u,c))}}}}function L(t){if(t?.includes("(")){let e=t.match(/[^\(\)]{1,}/g)
t=e?.[e.length-1]||""}return t?.split(N)?.map((t=>t.trim()))||[]}let F={store:(store_name,store_ops)=>(store_ops?.hasOwnProperty("value")||store_ops?.hasOwnProperty("updater")||(store_ops={value:store_ops}),p(store_name,store_ops)),ustore:(store_name,store_ops)=>p(store_name,store_ops),get:store_name=>p(store_name),func:func_name=>MfFn?.get(func_name),funcs:funcs=>{for(let t in funcs)MfFn.set(t,funcs[t])},config:(new_ops,profile_name)=>_(new_ops,profile_name),onTick:t=>{var e;(e=t)&&o.push(e)},register:t=>{"string"==typeof t&&(t=document.querySelector(t)),k(t)}}
globalThis.Mfld=F
