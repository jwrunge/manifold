...
"mount"==e?r():t.addEventListener(e,r)}const $={store:(store_name,store_ops)=>s(store_name,store_ops),ustore:(store_name,store_ops)=>s(store_name,store_ops),getFunc:func_name=>i.get(func_name),addFuncs:funcs=>{for(let t in funcs)i.set(t,funcs[t])},config:(new_ops,profile_name)=>b(new_ops,profile_name),tick:t=>u(t)}
globalThis.Cu=$
