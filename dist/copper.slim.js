...
if(t?.config)try{w(JSON.parse(t?.config))}catch(t){console.warn("Invalid Cu params",t)}t?.init&&function(t){let e=t?.querySelectorAll(`[data-${g.join("],[data-")}]${0!=m.fetch?.auto?",a":""}`)||[]
...
let m={},g=["bind","sync","fetch"]
function w(t,e){e?m.profiles={...m.profiles,[e]:t}:m={...m,...t}}function b(t,e,o){let n=t
...
"mount"==e?r():t.addEventListener(e,r)}const $={store:(store_name,store_ops)=>s(store_name,store_ops),ustore:(store_name,store_ops)=>s(store_name,store_ops),getFunc:func_name=>n.get(func_name),addFuncs:funcs=>{for(let t in funcs)n.set(t,funcs[t])},config:(new_ops,profile_name)=>w(new_ops,profile_name)}
globalThis.Cu=$
