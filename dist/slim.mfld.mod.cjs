...
t=e?.[e.length-1]||""}return t?.split(E)?.map((t=>t.trim()))||[]}globalThis.addEventListener("popstate",(t=>{}))
let O={store:(store_name,store_ops)=>(store_ops?.hasOwnProperty("value")||store_ops?.hasOwnProperty("updater")||(store_ops={value:store_ops}),b(store_name,store_ops)),ustore:(store_name,store_ops)=>b(store_name,store_ops),get:store_name=>b(store_name),func:func_name=>MfFn?.get(func_name),funcs:funcs=>{for(let t in funcs)MfFn.set(t,funcs[t])},config:(new_ops,profile_name)=>{return t=new_ops,void((e=profile_name)?S.profiles={...S.profiles,[e]:t}:S={...S,...t})
var t,e},onTick:t=>{var e;(e=t)&&c.push(e)},register:t=>{"string"==typeof t&&(t=document.querySelector(t)),_(t)}}
exports.Mfld=O
