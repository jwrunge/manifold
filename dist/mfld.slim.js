...
u.innerHTML=r,p({in:u,out:i,relation:"append",ops:o,done:t=>k(t)})}))}))}}let A={},E=/, {0,}/g,N=0,O=["bind","sync","if","each","get","head","post","put","delete","patch"].map((e=>`${t}${e}`))
function _(t,e){e?A.profiles={...A.profiles,[e]:t}:A={...A,...t}}function k(e){let n=(e||document.body).querySelectorAll(`[data-${O.join("],[data-")}]${0!=A.fetch?.auto?",a,form":""}`)||[]
...
t=e?.[e.length-1]||""}return t?.split(E)?.map((t=>t.trim()))||[]}let L={store:(store_name,store_ops)=>(store_ops?.hasOwnProperty("value")||store_ops?.hasOwnProperty("updater")||(store_ops={value:store_ops}),v(store_name,store_ops)),ustore:(store_name,store_ops)=>v(store_name,store_ops),get:store_name=>v(store_name),func:func_name=>MfFn?.get(func_name),funcs:funcs=>{for(let t in funcs)MfFn.set(t,funcs[t])},config:(new_ops,profile_name)=>_(new_ops,profile_name),onTick:t=>{var e;(e=t)&&u.push(e)},register:t=>{"string"==typeof t&&(t=document.querySelector(t)),k(t)}}
globalThis.Mfld=L
