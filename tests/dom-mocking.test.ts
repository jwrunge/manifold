...
describe("DOM behavior / structural stability", () => {
	...
	test("updating individual array item preserves sibling node identities (case 10)", async () => {
		...
		RegEl.register(template as HTMLElement, state);
		await flush();
		const items = () => Array.from(ul.querySelectorAll("li:not([data-each])"));
		const nodes = items();
	...
	test("interpolation inside each clones (case 11)", async () => {
		...
		RegEl.register(template as HTMLElement, state);
		await flush();
		const texts = Array.from(root.querySelectorAll("p:not([data-each])")).map(
	...
	test("then/catch must follow await (case 17)", async () => {
		...
		await flush();
		expect(
			(document.getElementById("prematureThen") as HTMLElement).style.display,
		).toBe("none");
		await multiFlush(5);
		expect(
			(document.getElementById("prematureThen") as HTMLElement).style.display,
