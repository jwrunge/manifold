...
if(t?.config)try{m(JSON.parse(t?.config))}catch(t){console.warn("Invalid Cu params",t)}t?.init&&function(t){let e=t?.querySelectorAll(`[data-${g.join("],[data-")}]${0!=w.fetch?.auto?",a":""}`)||[]
for(let t of e){t.id||(t.id="cu-"+y++)
for(let e in t.dataset){if(!g.includes(e))continue
...
a?.length||(a=[""])
for(let o of a){"fetch"==e&&T(t,o,h,d,w),d?.length||(d=[""])
...
let w={},g=["bind","sync","fetch"]
function m(t,e){e?w.profiles={...w.profiles,[e]:t}:w={...w,...t}}function b(t,e,o){let n=t
...
"mount"==e?r():t.addEventListener(e,r)}const $={store:(store_name,store_ops)=>s(store_name,store_ops),ustore:(store_name,store_ops)=>s(store_name,store_ops),getFunc:func_name=>n.get(func_name),addFuncs:funcs=>{for(let t in funcs)n.set(t,funcs[t])},config:(new_ops,profile_name)=>m(new_ops,profile_name),tick:t=>u(t)}
export{$ as Cu}
