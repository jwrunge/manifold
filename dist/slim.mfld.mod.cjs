...
return e?n?n.p(t,e):new p(t,e):n||new p(t,e)}let $="mf",h=/[\.\[\]\?]{1,}/g
...
return s||(i.match(/^\s{0,}\{/)||i.includes("return")||(i=i.replace(/^\s{0,}/,"return ")),s=new Function(...r,i)),{storeList:r,func:s,storeName:n}}function T(t,e,n,o,i,r){let l=async e=>{e?.preventDefault(),e?.stopPropagation()
let l=y(n,t)
...
"$mount"==e?l():t.addEventListener(e,l)}function b(t,e,n,o,i,r){if(i==`${$}bind`){let i=e.map(w),l=()=>{s((()=>{let e=i.map((t=>m(d(t[0])?.value,t[1]))),s=r?.(...e,t)??e[0]
n&&void 0!==s&&(t[n]=s),t.dispatchEvent(new CustomEvent(o))}))}
for(let e of i)d(e?.[0]||"")?.sub(l,t.id)}else if(i==`${$}sync`){let[i,s]=w(n||""),l=()=>{let n=e.map((e=>(e=e.trim(),t[e]??t.getAttribute(e)??t.dataset[e]??void 0))),o=r?.(...n)??n[0]
i&&void 0!==o&&d(i)?.update?.((t=>s?.length?m(t,s,o):o))}
...
return t.replaceWith(n),e.innerHTML=t.innerHTML,n}return t}function M(t,e,n,o,i){return d(t||"",{upstream:[...e||[],...n||[]],updater:t=>{if(o)for(let e of t.slice(-o)||[])if(e)return!1
...
console.log("Got replacements",f)
for(let t of f){let o=t.replace(/^\$\{|\}$/g,"")
try{let i=v(`(${l}, ${r})=> ${o}`)?.func
console.log("LOOP FN SETUP",l,`(${l}, ${r})=> return ${o}`,i),i?.(n,e),a=a.replace(t,i?.(n,e)||"")}catch(t){console.error("Syntax error in loop function",t)}}let u=document.createElement("div")
u.innerHTML=a,s({in:u,out:i,relation:"append",ops:o,done:t=>_(t)})}))}))}}let N={},E=/, {0,}/g,L=0,O=["bind","sync","if","each","get","head","post","put","delete","patch"].map((t=>`${$}${t}`))
function _(t){let e=(t||document.body).querySelectorAll(`[data-${O.join("],[data-")}]${0!=N.fetch?.auto?",a":""}`)||[]
for(let t of e){t.id||(t.id=""+L++)
for(let e in t.dataset){if([`${$}if`,`${$}each`].includes(e)){S(t,e,N)
continue}if(!O.includes(e))continue
let n=![`${$}bind`].includes(e),o=`(#${t.id} on ${e})`
t.dataset?.[e]?.split(";").forEach((i=>{let[r,s]=i?.split("->")?.map((t=>t.trim()))||[],l=n?A(r.slice(0,r.indexOf(")"))):[],a=n?r.slice(r.indexOf(")")+1):r,f=a.includes("=>")?a:a.includes("(")&&a.match(/^[^\(]{1,}/)?.[0]||"",u=f?A(a.slice(0,(a.indexOf(")")||-2)+1)):a.split(E)?.map((t=>t.trim()))
...
for(let n of l)e.match(/bind|sync/)?b(t,u,s,n,e,c):T(t,n,N,u[0],e.replace($,""),s)}))}}}function A(t){if(t?.includes("(")){let e=t.match(/[^\(\)]{1,}/g)
t=e?.[e.length-1]||""}return t?.split(E)?.map((t=>t.trim()))||[]}let P={store:(store_name,store_ops)=>(store_ops?.hasOwnProperty("value")||store_ops?.hasOwnProperty("updater")||(store_ops={value:store_ops}),d(store_name,store_ops)),ustore:(store_name,store_ops)=>d(store_name,store_ops),get:store_name=>d(store_name),func:func_name=>MfFn?.get(func_name),funcs:funcs=>{for(let t in funcs)MfFn.set(t,funcs[t])},config:(new_ops,profile_name)=>{return t=new_ops,void((e=profile_name)?N.profiles={...N.profiles,[e]:t}:N={...N,...t})
