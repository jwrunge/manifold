...
"swapinner"==t.relation&&(e.style.border="none",t.out.replaceChildren()),u(e,"out",t.ops,void 0,t.out,n)}l?.(t.in,t.out,e,!1,t.ops),u(t.in,"in",t.ops,(()=>{t.in&&("swapouter"==t.relation?t.out?.replaceWith(t.in):t.out?.appendChild(t.in)),a?.(t.in,t.ops)}))}t.done?.(t.in)}for(let t of o)t()
o=[],e=[]}function u(t,e,n,o,i,r=!1){if(t?.nodeType==Node.TEXT_NODE){let e=t.textContent,n=document?.createElement("div")
...
l=new Set
u=new Set
constructor(t,e){return this.p(t,e)}p(t,e){return this.name=t,MfSt.set(t,this),e?.upstream?.map((t=>{let e=p(t)
return this.l.add(e),e.u.add(this),e})),this.value=e?.value,this.t=e?.updater,this.$(),this}sub(t,e,n=!0){this.o.set(e||String(Date.now()+Math.random()),t),n&&t?.(this.value)}async update(t){return new Promise((async e=>{let n="function"==typeof t?(await t)?.(this.value):t,o=c(n)
...
return[e,n?.map((t=>isNaN(parseInt(t))?t:parseInt(t))).filter((t=>t))||[]]}function y(t,e,n,o=!0,i="{}",r){let s=e.dataset[`${$}${t}`]
if(s)return"overrides"==t?n.profiles?.[s||""]?.fetch||JSON.parse(s||"{}"):o?JSON.parse(s||i):"num"==r?parseInt(s)||void 0:"bool"==r?"true"==s||"false"!=s&&void 0:s}function g(t,e){let n=y("overrides",e,t)
return{profiles:t.profiles,fetch:{...t.fetch,responseType:y("responsetype",e,t,!1)||t.fetch?.responseType,...n?.fetch||{},...y("fetch",e,t)||{}},trans:{...t.trans,dur:y("transdur",e,t,!0,"[]","num")||t.trans?.dur,swap:y("transswap",e,t,!1,"","num")||t.trans?.swap,class:y("transclass",e,t,!1)||t.trans?.class,smartTransition:y("transsmart",e,t,!1,void 0,"bool")||t.trans?.smartTransition,...n?.trans||{},...y("trans",e,t)||{}}}}function v(t){let e="",n=""
...
return s||(i.match(/^\s{0,}\{/)||i.includes("return")||(i=i.replace(/^\s{0,}/,"return ")),s=new Function(...r,i)),{storeList:r,func:s,storeName:n}}function b(t,e,n,o,i,r,l){let a=async e=>{e?.preventDefault(),e?.stopPropagation()
let a=g(n,t)
i||(i=(e?.target)?.method||"get"),a?.fetch?.externals?.find((t=>o?.startsWith(t.domain)))||!o.match(/^https?:\/\//)||o.includes(location.origin)
let f=r
l&&(f=l?.(...r?.map((t=>p(t).value))||[]))
...
r&&s({in:r.querySelector(o||"body"),out:i?document.querySelector(i):t,relation:e,ops:a,done:t=>{k(t)}})}let h=t.dataset?.[`${$}resolve`],m=v(h||"")?.func
m?.(d)}
"$mount"==e?a():t.addEventListener(e,a)}function T(t,e,n,o,i,r){if(i==`${$}bind`){let i=e.map(w),l=()=>{s((()=>{let e=i.map((t=>m(p(t[0])?.value,t[1]))),s=r?.(...e,t)??e[0]
if(n&&void 0!==s){let e=n.split(":")
if(e.length>1)switch(e[0]){case"style":t.style[e[1]]=s
break
case"attr":t.setAttribute(e[1],s)
break
default:t[n]=s}else t[n]=s}t.dispatchEvent(new CustomEvent(o))}))}
for(let t of i)p(t?.[0]||"")?.sub(l)}else if(i==`${$}sync`){let[i,s]=w(n||""),l=()=>{let n=e.map((e=>{let n=(e=e.trim()).split(":")
if(!(n.length>1))return t[e]??void 0
switch(n[0]){case"style":return t.style[e]??void 0
case"attr":return t.getAttribute(e)??void 0
default:return t[e]??void 0}})),o=r?.(...n)??n[0]
...
return i?.(...t)}})}function S(t,e,n){let o=g(n,t),i=document.createElement("div")
...
e.innerHTML=u.innerHTML,"TEMPLATE"==u?.tagName&&s({in:e,out:i,relation:"swapinner",ops:o,done:t=>k(t)})})),n=n?.nextElementSibling}}if(e==`${$}each`){i.classList.add("mfld-loop-result")
...
u.innerHTML=a,s({in:u,out:i,relation:"append",ops:o,done:t=>k(t)})}))}))}}let E={},N=/, {0,}/g,A=0,_=["bind","sync","if","each","get","head","post","put","delete","patch"].map((t=>`${$}${t}`))
function k(t){let e=(t||document.body).querySelectorAll(`[data-${_.join("],[data-")}]${0!=E.fetch?.auto?",a":""}`)||[]
for(let t of e){t.id||(t.id=""+A++)
for(let e in t.dataset){if([`${$}if`,`${$}each`].includes(e)){S(t,e,E)
continue}if(!_.includes(e))continue
