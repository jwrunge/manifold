...
return t.replaceWith(e),e}return t}function x(t,e,n){return e(t)?t:(t=n?.(t)||t,x(t?.nextElementSibling,e,n))}function S(t,e,n,o,i){return v(t||"",{upstream:[...e||[],...n||[]],updater:t=>{if(o)for(let n of t.slice(e?.length||0)||[])if(n)return!1
let r=i?.(...t)
...
if(i.classList.add("mfld-start"),r.classList.add("mfld-end"),e.before(i),n==`${t}if`){let e=[],s=e.length,f=x(i?.nextElementSibling,(e=>null==e?.dataset[`${t}if`]&&null==e?.dataset[`${t}elseif`]&&null==e?.dataset[`${t}else`]))
if(null==f?.previousElementSibling?.dataset[`${t}else`]){let e=document.createElement("template")
e.dataset[`${t}else`]="()=> true",e.innerHTML="<div>VISIBLE IF NO CONDITIONS</div>",sib.after(e)}x(i?.nextElementSibling,(e=>{let n=0
...
f=M(f)
let d=S(c,a,e,s,u)
return e.push(d.name),d?.sub((t=>{t&&(p((()=>{x(i?.nextElementSibling,(t=>t?.classList?.contains("mfld-end")),(t=>{"TEMPLATE"!=t?.nodeName&&w(t,"out",o,(()=>t?.remove()))}))})),p((()=>{let t=f.cloneNode(!0)
...
function F(e){if(e&&e.nodeType==Node.TEXT_NODE)return
...
for(let r of e.dataset?.[o]?.split(";")||[]){let[s,f]=r?.split("->")?.map((t=>t.trim()))||[],a=i?I(s.slice(0,s.indexOf(")"))):[],u=i?s.slice(s.indexOf(")")+1):s,c=u.includes("=>")?u:u.includes("(")&&u.match(/^[^\(]{1,}/)?.[0]||"",d=c?I(u.slice(0,(u.indexOf(")")||-2)+1)):u.split(A)?.map((t=>t.trim()))
...
for(let i of a)o.match(/bind|sync/)?T(e,d,f,i,o,p):(f||(f=d[0],d=[]),b(e,i,n,f,o.replace(t,""),d,p))}}}}function I(t){if(t?.includes("(")){let e=t.match(/[^\(\)]{1,}/g)
...
var t,e},onTick:t=>{var e;(e=t)&&u.push(e)},register:t=>{"string"==typeof t&&(t=document.querySelector(t)),F(t)}}
export{_ as Mfld}
