...
if("prepend"==t.relation)u?.(t.in,t.out,e,t.ops),d(t.in,"in",t.ops,(()=>{t.out?.prepend(t.in),a?.(t.in,t.ops)}))
else{if(["inner","outer"].includes(t.relation)){let e=t.out?.cloneNode(!0)
e&&(t.out?.after(e),i&&(e.style.border="none",t.out.replaceChildren()),d(e,"out",t.ops,void 0,t.out,i))}u?.(t.in,t.out,e,t.ops),d(t.in,"in",t.ops,(()=>{"outer"==t.relation?t.out?.replaceWith(t.in):t.out?.appendChild(t.in),a?.(t.in,t.ops)}))}t.done?.(t.in)}l.forEach((t=>t())),l=[],r=[]},d=(e,i,n,o,r,s=!1,l)=>{if(e?.nodeType==Node.TEXT_NODE&&(e.replaceWith(document?.createElement("div")),e.textContent=e.textContent),e){const u=Array.isArray(n.trans?.dur)?n.trans?.dur["in"==i?0:1]||n.trans?.dur[0]:n.trans?.dur||0,a=n?.trans?.class||`${t}trans`
if(e?.classList?.add(a),n.trans?.hooks?.[`${i}-start`]?.(e),"out"==i){if(!(r=r||e))return
let t={};(n.trans?.smart??1)&&!s&&(t=h(r)),f((()=>{(n.trans?.smart??1)&&s&&r&&(t=h(r)),(n.trans?.smart??1)&&(e.style.position="fixed",e.style.width=t.w,e.style.left=t.left,e.style.top=t.top,e.style.margin="0"),u&&(e.style.transitionDuration=`${u}ms`),e.classList?.add("out")}))}else e?.classList?.add("in"),u&&(e.style.transitionDuration=`${u}ms`),o?.(),setTimeout((()=>{f((()=>{setTimeout((()=>f((()=>e?.classList?.remove(i)))),0)}))}),n.trans?.swap||0)
setTimeout((()=>{f((()=>{"out"==i&&e?.remove(),e?.classList?.remove(a),n.trans?.hooks?.[`${i}-end`]?.(e),e.style.transitionDuration="","in"==i&&l?.(e)}))}),u+("in"==i&&n.trans?.swap||0))}},h=t=>{let e=getComputedStyle(t),i=t.getBoundingClientRect()
...
return e?.(t)?t:w((i?.(t)||t)?.[o],e,i,n)}let y=(t,e)=>p(i(),{upstream:[...t||[]],updater:()=>e?.func?.(e.observeEl,S,E),scope:e?.observeEl}),b=(t,e,i,n)=>{y(n,{observeEl:t,func:()=>(t.dispatchEvent(new CustomEvent(e)),i?.(t,S,E))})},_=(e,i,n)=>{let o=i=>{n?.(e,S,E),function(e,i,n){i?.preventDefault()
let o=e.dataset?.[`${t}pushstate`],r=n
switch(o){case"":break
case void 0:return
default:r=`#${o}`}history.pushState(null,"",r)}(e,i)}
"$mount"==i?o():e.addEventListener(i,o)},x=(e,i,n,r,s,l)=>{let u,a,c=document.createElement("template"),h=(t=>{let e="TEMPLATE"
...
if(c.classList.add(`${i}-start`),h.classList.add(`${i}-end`),e.before(c),e.after(h),e.remove(),p){if($){let e=w(c,(e=>e?.classList?.contains(`${t}if-end`)),null,!0)
w(e,(t=>t==h),(e=>{e?.dataset?.[`${t}cstore`]&&m.push(e?.dataset?.[`${t}cstore`])}))}a=(...t)=>{if($)for(let e of t.slice(-m.length))if(1==e)return!1
return"else"==$?.[0]||1==r?.(...t)}}u=y(s,{func:p?a:r,observeEl:h}),p&&(h.dataset[`${t}cstore`]=u.name),u.sub((t=>{void 0!==t&&f((()=>{w(c?.nextElementSibling,(t=>t?.classList?.contains(`${i}-end`)),(t=>d(t,"out",l,(()=>t?.remove())))),p&&!t||((t,e)=>{if(t instanceof Map)for(const[i,n]of t.entries())e(i,n)
else try{let i=Array.from(t||[])
if(i?.length)i.forEach(e)
else for(let i in t)e(i,t[i])}catch(e){console.error(`${t} is not iterable`)}})(i.match(/each/)?t:[t],((t,i)=>{let r=h.cloneNode(!0)
if(!p){let s=h?.innerHTML?.replace(/\$:{([^}]*)}/g,((r,s)=>o(s,n[0],n[1])?.func?.(e,S,E,t,i)||""))||""
r?.innerHTML&&(r.innerHTML=s)}for(let e of r.content.children)e?.innerHTML||(e.innerHTML=t),h.before(e),d(e,"in",l)}))}))}))},M={},T=["bind","sync","templ","if","elseif","else","each","get","head","post","put","delete","patch"].map((e=>`${t}${e}`))
$.addEventListener("popstate",(()=>{location.reload()}))
let{$fn:E,$st:S}=$.MFLD,A={store:
/**!
 ...
 */
(new_ops,profile_name)=>{return t=new_ops,void((e=profile_name)?M.profiles={...M.profiles,[e]:t}:M={...M,...t})
var t,e},onTick:
/**!
...
let s=(r||document.body).querySelectorAll(`[data-${T.join("],[data-")}],a,form`)
...
if(e)continue}for(let t in r.dataset)if(T.includes(t))for(let i of r.dataset?.[t]?.split(";;")||[]){let[n,l]=i?.split(/\s*->\s*/).reverse(),f=l?.match(/[^\(\)]{1,}/g)?.pop()?.split(e)?.map((t=>t.trim())),u=[...n?.matchAll(/\$st\.(\w{1,})/g)].map((t=>t[1]))
console.log(r,f,n,u)
let{func:a,as:c}=o(n)
if(t.match(/each|templ|if|else/))x(r,t,c||[],a,u,s)
else{f?.length||(f=[""])
for(let e of f)t.match(/bind/)?b(r,e,a,u):t.match(/sync/)&&_(r,e,a)}}}})(r)}},F=$.MFLD.$st,L=$.MFLD.$fn
