...
function h(t){f.push(t),u||(u=!0,p(w))}function m(t,e,n,i=!1,o){if(!o.trans?.smartTransition??1)return
a=document.createElement("div")
let{paddingTop:r,paddingBottom:s}=e instanceof Element?window.getComputedStyle(e):{paddingTop:0,paddingBottom:0}
d=a.style.height=`calc(${Math.abs(n-(t?.clientHeight||0))}px - ${r} - ${s})`,e?.after(a)}function $(t,e){if(!e.trans?.smartTransition??1)return
...
if("prepend"==t.relation)m?.(t.in,t.out,e,!1,t.ops),g(t.in,"in",t.ops,(()=>{t.in&&t.out?.prepend(t.in),$?.(t.in,t.ops)}))
else{if(["swapinner","swapouter"].includes(t.relation)){let e=t.out?.cloneNode(!0)
if(e){t.out?.after(e)
let n="swapinner"==t.relation
"swapinner"==t.relation&&(e.style.border="none",t.out.replaceChildren()),g(e,"out",t.ops,void 0,t.out,n)}}m?.(t.in,t.out,e,!1,t.ops),g(t.in,"in",t.ops,(()=>{t.in&&("swapouter"==t.relation?t.out?.replaceWith(t.in):t.out?.appendChild(t.in)),$?.(t.in,t.ops)}))}t.done?.(t.in)}for(let t of c)t()
...
setTimeout((()=>{h((()=>{"out"==n&&e?.remove(),e?.classList?.remove(a),i.trans?.hooks?.[`${n}-end`]?.(e)}))}),l+("in"==n&&i.trans?.swap||0))}}function y(t){if(!t)return 0
...
return this.l.add(e),e.u.add(this),e})),this.value=e?.value,this.t=e?.updater,this.m(),this}sub(t,e,i=!0){this.i.set(e||n(),t),i&&t?.(this.value)}async update(t){return new Promise((async e=>{let n="function"==typeof t?(await t)?.(this.value):t,i=y(n)
...
return e?n?n.h(t,e):new v(t,e):n||new v(t,e)}function M(t){t.i.clear(),t.l.clear(),t.u.clear(),MfSt.delete(t.name),t=void 0}function T(e,n,i,o,r,s,a){let f=async n=>{n?.preventDefault(),n?.stopPropagation(),r||(r=(n?.target)?.method||"get"),i?.fetch?.externals?.find((t=>o?.startsWith(t.domain)))||!o.match(/^https?:\/\//)||o.includes(location.origin)
...
let p=e.dataset?.[`${t}resolve`],m=l(p||"")?.func
m?.(d)}
...
"$mount"==l?a():e.addEventListener(l,a)}var u,c}let S={},N=/, {0,}/g,E=["bind","sync","templ","if","each","get","head","post","put","delete","patch"].map((e=>`${t}${e}`))
function O(t,e){e?S.profiles={...S.profiles,[e]:t}:S={...S,...t}}function _(e){if(e&&e.nodeType==Node.TEXT_NODE)return
let n=(e||document.body).querySelectorAll(`[data-${E.join("],[data-")}],a,form`)||[]
...
if(i){T(e,r,n,i,t,o)
continue}}for(let n in e.dataset){if(!E.includes(n))continue
...
let F={store:(store_name,store_ops)=>(store_ops?.hasOwnProperty("value")||store_ops?.hasOwnProperty("updater")||(store_ops={value:store_ops}),b(store_name,store_ops)),ustore:(store_name,store_ops)=>b(store_name,store_ops),get:store_name=>b(store_name),func:func_name=>MfFn?.get(func_name),funcs:funcs=>{for(let t in funcs)MfFn.set(t,funcs[t])},config:(new_ops,profile_name)=>O(new_ops,profile_name),onTick:t=>{var e;(e=t)&&c.push(e)},register:t=>{"string"==typeof t&&(t=document.querySelector(t)),_(t)}}
globalThis.Mfld=F
let L=globalThis.document?.currentScript?.dataset||{}
if(L?.config)try{O(JSON.parse(L?.config))}catch(t){console.warn("Invalid Mfld params",t)}L?.init&&_(document.querySelector(L.init))
