...
n.done?.(n.in)}o.forEach((t=>t())),o=[],e=[]}function l(e,n,o,i){if(e?.nodeType==Node.TEXT_NODE){let t=e.textContent,n=document?.createElement("div")
...
e?.classList?.add(l),o.trans?.hooks?.[`${n}-start`]?.(e),"out"==n?r((()=>{t?.size?.(e),f&&(e.style.transitionDuration=`${f}ms`),e.classList?.add(n)})):setTimeout((()=>{r((()=>{f&&(e.style.transitionDuration=`${f}ms`),e?.classList?.add(n),i?.(),r((()=>{e?.classList?.remove(n)}))}))}),o.trans?.swap||0),setTimeout((()=>{r((()=>{"out"==n&&e?.remove(),e?.classList?.remove(l),e?.classList?.remove(n),o.trans?.hooks?.[`${n}-end`]?.(e)}))}),f+("in"==n&&o.trans?.swap||0))}}function s(t){if(!t)return 0
...
t.dataset?.[e]?.split(";").forEach((i=>{let f,l=i?.split(/(?:(?:\)|->) ?){1,}/g)||[],s=n?w(l.splice(0,1)[0]):[],a=l[0]?.includes("(")&&l[0]?.match(/^[^\(]{1,}/)?.[0]||"",u=w(l.splice(e==`${d}sync`?1:0,1)[0]),h=w(l[0])
