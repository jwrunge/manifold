...
try{u=new Function(...f,s)}catch(t){console.error(t)}}return{valueList:f,func:u,as:a}}let l,a=[],f=!1,u=[],c="",d=globalThis?.requestAnimationFrame||(t=>setTimeout(t,0))
function p(t){a.push(t),f||(f=!0,d($))}function h(t,e,n,o=!1,i){if(!i.trans?.smartTransition??1)return
...
setTimeout((()=>{p((()=>{"out"==n&&e?.remove(),e?.classList?.remove(a),o.trans?.hooks?.[`${n}-end`]?.(e)}))}),l+("in"==n&&o.trans?.swap||0))}}function g(t){if(!t)return 0
...
return this.l.add(e),e.u.add(this),e})),this.value=e?.value,this.t=e?.updater,this.$(),this}sub(t,e,n=!0){this.o.set(e||o(),t),n&&t?.(this.value)}async update(t){return new Promise((async e=>{this.h&&clearTimeout(this.h),this.h=setTimeout((()=>{p((async()=>{let n="function"==typeof t?(await t)?.(this.value):t,o=g(n)
...
return e?n?n.m(t,e):new y(t,e):n||new y(t,e)}function v(t){t.o.clear(),t.l.clear(),t.u.clear(),MfSt.delete(t.name),console.log("Cleared store",t.name),t=void 0}function M(e,n,o,i,r,l,a){let f=async n=>{n?.preventDefault(),n?.stopPropagation(),r||(r=(n?.target)?.method||"get"),o?.fetch?.externals?.find((t=>i?.startsWith(t.domain)))||!i.match(/^https?:\/\//)||i.includes(location.origin)
...
l&&p({in:l.querySelector(r||"body"),out:s?document.querySelector(s):e,relation:n,ops:o,done:t=>{N(t)}})}void 0!==e.dataset?.[`${t}pushstate`]&&history.pushState({},"",i)
let m=e.dataset?.[`${t}resolve`],$=s(m||"")?.func
$?.(h)}
"$mount"==n?f():e.addEventListener(n,f)}function T(t,e,n=[],o=[]){if(t.tagName!=e){let i=document.createElement(e)
...
return t.replaceWith(i),i}return t}function x(t,e){if(t instanceof Map)for(const[n,o]of t.entries())e(n,o)
else try{let n=Array.from(t)
if(n?.length)n.forEach(e)
else for(let n in t)e(n,t[n])}catch(e){console.error(`${t} is not iterable`)}}function S(t,e,n){return e?.(t)?t:(t=n?.(t)||t,S(t?.nextElementSibling,e,n))}function E(t,e,n){return b(t||"",{upstream:[...e||[]],updater:t=>{try{return n?.func?.(...t)||t[0]}catch(t){return}},scope:n?.observeEl})}function _(t,e,n,i,r,s){if(r.match("bind"))E(o(),e,{observeEl:t,func:()=>{let o=s?.(...function(t){let e=[]
...
"$mount"==i?o():t.addEventListener(i,o)}}function A(e,n,i,r,l,a){let f=document.createElement("template"),u=T(e.cloneNode(!0),"TEMPLATE")
f.classList.add(`${n}-start`),u.classList.add(`${n}-end`),u.dataset.nodeName=e.nodeName,e.before(f),e.after(u),e.remove(),E(o(),l,{func:r,observeEl:u}).sub((e=>{p((()=>{S(f?.nextElementSibling,(t=>t?.classList?.contains(`${n}-end`)),(t=>w(t,"out",a,(()=>t?.remove())))),(n.match(/each/)?x:(t,e)=>e(t||""))(e,((e,o)=>{if(null==e)return
let r,l=u?.innerHTML||u?.textContent?.replace(/^\n{0,}|\n{0,}$/,"")||"",f=l.match(/\${[^}]*}/g)||[]
for(let t of f)try{let n=s(`(${i.join(",")})=> ${t.slice(2,t.length-1)}`)?.func
l=l.replace(t,n?.(e,o)||"")}catch(t){throw t}if(n.match(/each/)){let t=u.cloneNode(!0)
t.innerHTML=l||e,r=t.content.children}else{let o=T(u.cloneNode(!0),u.dataset.nodeName,["data-node-name",`data-${t}`],[`${n}-end`])
o.innerHTML=l||e,r=[o]}for(let t of r)u.before(t),w(t,"in",a,(()=>N(t)))}))}))}))}let O={},F=["bind","sync","templ","if","each","get","head","post","put","delete","patch"].map((e=>`${t}${e}`))
...
if(u&&!c&&console.warn(`"${u}" not registered`,e),o.match(/if|each|templ/))A(e,o,p||[],c,d||[],n)
else{f?.length||(f=[""])
for(let i of f)o.match(/bind|sync/)?_(e,d,a,i,o,c):M(e,i,n,a,o.replace(t,""),d,c)}}}}}function k(t){if(t?.includes("(")){let e=t.match(/[^\(\)]{1,}/g)
 ...
 */
(new_ops,profile_name)=>{return t=new_ops,void((e=profile_name)?O.profiles={...O.profiles,[e]:t}:O={...O,...t})
