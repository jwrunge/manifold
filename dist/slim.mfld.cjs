...
t?.dataset?.[e]?.split(";").forEach((r=>{let s,l=r?.split(/(?:(?:\)|->) ?){1,}/g)||[],a=o?v(l.splice(0,1)[0]):[],u=l[0]?.includes("(")&&l[0]?.match(/^[^\(]{1,}/)?.[0]||"",h=v(l.splice("sync"==e?1:0,1)[0]),d=v(l[0])
...
a?.length||(a=[""])
for(let o of a){"fetch"==e&&T(t,o,h,d,g),d?.length||(d=[""])
...
t=e?.[e.length-1]||""}return t?.split(w)||[]}function T(t,e,o,i,n){let r=o=>{o?.preventDefault(),o?.stopPropagation()
