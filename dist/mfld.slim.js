...
c(t)?.l.has(this.name||"")&&n.add(t)}let o="function"==typeof t?(await t)?.(this.value):t,i=s(o)
...
function $(t,e){e?h.profiles={...h.profiles,[e]:t}:h={...h,...t}}function w(t){let e=(t||document.body).querySelectorAll(`[data-${p.join("],[data-")}]${0!=h.fetch?.auto?",a":""}`)||[]
...
if(a){if(l=globalThis.MfFn?.get(a),!l)throw`"${a}" not registered: ${o}`
...
for(let i=0;i<h.length;i++)if(e==`${d}bind`){let e=()=>{r((()=>{let e=l?.(...p.map((t=>y(c(t.name)?.value,t.path))),t)??y(c(p[0].name||"")?.value,p[0].path)
...
l&&(e=l?.(e,t))
let n=c(p[0]?.name)
void 0!==e&&n?.update?.((t=>p[0]?.path?.length?y(t,p[0]?.path,e):e))}
t.addEventListener(n,e)}}}))}}}function y(t,e,n){let o=t
