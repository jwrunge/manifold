...
t?.dataset?.[e]?.split(";").forEach((n=>{let s,r=n?.split(/(?:(?:\)|->) ?){1,}/g)||[],f=o?y(r.splice(0,1)[0]):[],u=r[0]?.includes("(")&&r[0]?.match(/^[^\(]{1,}/)?.[0]||"",c=y(r.splice("sync"==e?1:0,1)[0]),h=y(r[0])
...
f?.length||(f=[""])
for(let o of f){"fetch"==e&&m(t,o,c,h,b),h?.length||(h=[""])
for(let n=0;n<h.length;n++)if("bind"==e){let e=()=>{a((()=>{t[h[n]]=s?.(...d.map((t=>w(l(t.name)?.value,t.path))),t)??w(l(d[0].name||"")?.value,d[0].path),t.dispatchEvent(new CustomEvent(o))}))}
